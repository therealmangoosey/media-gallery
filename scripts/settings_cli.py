...
def load():
    ...
    if SETTINGS.exists():
        ...
        m(d,r)
    d.setdefault('server',{}).setdefault('host','0.0.0.0')
...
def power(d):
    ...
    cur=get(d,'runtime.power_mode','balanced'); mode=ask('Power mode (eco/balanced/full)',cur).lower()
    if mode not in modes: return print('Choose eco, balanced or full.')
    setv(d,'runtime.power_mode',mode); x=modes[mode]
    setv(d,'gallery.page_size',x['page_size']); setv(d,'uploads.max_file_mb',x['max_file_mb']); setv(d,'moderation.enabled',x['moderation']); setv(d,'gallery.allow_voting',x['voting']); setv(d,'gallery.allow_search',x['search']); print('Power profile applied. You can fine-tune settings afterwards.')

def main():
    d=load()
    while True:
        print('\n=== Media Gallery — Termux Control Panel ==='); print('1) Power mode\n2) Site appearance\n3) Server + storage\n4) Uploads\n5) Gallery / search / voting\n6) Moderation\n7) Accounts\n8) Cloudflare Turnstile\n9) Discord webhook\n10) Security\n11) Runtime / recovery\n12) Show settings\n13) Save\n0) Back')
        ...
        try:
            ...
            elif c=='11': group(d,'Runtime',[('runtime.auto_recover','Auto-recover startup failures','bool',None,None)]); print('Runtime is native Termux only. There is no container/proot mode.')
