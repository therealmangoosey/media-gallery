...
health(){ local host="$1" port="$2"; if command -v curl >/dev/null 2>&1; then curl -fsS --max-time 2 "http://127.0.0.1:$port/health" >/dev/null 2>&1; return $?; fi; python3 - "$port" <<'PY'
...
auto_recover="$(read_setting runtime.auto_recover true)"; configured_proot="$(read_setting runtime.use_proot false)"; listen_host="$(read_setting server.listen_host 0.0.0.0)"
...
if [ -f "$PID_FILE" ]; then old="$(cat "$PID_FILE" 2>/dev/null || true)"; if [[ "$old" =~ ^[0-9]+$ ]] && kill -0 "$old" 2>/dev/null && health "$listen_host" "$port"; then log "Gallery is already running (PID $old, port $port)."; exit 0; fi; rm -f "$PID_FILE"; fi
